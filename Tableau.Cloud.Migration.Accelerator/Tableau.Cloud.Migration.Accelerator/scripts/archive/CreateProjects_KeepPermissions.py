...
#           If project names have a slash / , the separator should be changed to double slash //
#           example //Parent/Child with / Slash//Grandchild
#
...
#   4) Project Owner (optional): email address of the project owner
#  ------------------------------------------------------------------------------------------------------------
#       A sample entry:
...
proj_dest_file = file_loc + "Create Projects Log.xlsx"
...
log_df["Script Name"] = "CreateProjects_KeepPermissions.py"
...
csv_df["Path"] = np.where(
    csv_df["Path"].str[0:1] != sep, sep + csv_df["Path"].astype(str), csv_df["Path"]
)
...
print("Number of Projects in input file: " + str(count_proj_source))
                ...
                if i == 1:
                    ...
                    if temp_current_proj_df.empty:
                        new_row = {
                            ...
                            "Script Name": "CreateProjects_KeepPermissions.py",
                            "Parent Project": proj_parent_name,
                            "Path": proj_path,
                            "Result": "Failed",
                            "Error": "Parent Project is not a row in the input file. This project was not "
                            "created",
                        }
                        log_df = pd.concat(
